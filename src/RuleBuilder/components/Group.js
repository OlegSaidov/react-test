...
class Group extends Component {
  static propTypes = {
    ...
    notOptions: PropTypes.object.isRequired,
  ...
  render () {
    const notOptions = this.props.notOptions
    return (
      <div className='group'>
        <div className='group--header'>
          <div className='group--conjunctions'>
            <div
              className='conjunction conjunction--NOT'
              data-state={notOptions.checked ? 'active' : 'inactive'}
            >
              <label htmlFor={notOptions.id}>Not</label>
              <input
                id={notOptions.id}
                name={notOptions.name}
                type='checkbox'
                checked={notOptions.checked}
                onChange={notOptions.setConjunction}
              />
            </div>
            {mapValues(
              (item, index) => (
