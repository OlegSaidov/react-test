...
import {
  ...
  defaultValueOptions
} from './utils/defaultRuleProperties'
...
const setConjunction = (state, path, conjunction) => {
  ...
  return state.updateIn(expandTreePath(path, 'properties', 'not'), false, v => !v)
}
 ...
 * @param {Immutable.List} path
 */
const removeItem = (state, path) => state.deleteIn(expandTreePath(path))
...
const setField = (state, path, field, config) =>
  state.updateIn(expandTreePath(path, 'properties'), map =>
    map.withMutations(current => {
      ...
      const operator =
        config.fields[field].operators.indexOf(currentOperator) !== -1
          ? currentOperator
...
const setOperator = (state, path, operator, config) =>
  state.updateIn(expandTreePath(path, 'properties'), map =>
    map.withMutations(current => {
      ...
      const nextValue = new Immutable.List(
        currentValue.take(operatorCardinality),
      )
...
const setValueOption = (state, path, delta, name, value) =>
  state.setIn(
    expandTreePath(path, 'properties', 'valueOptions', delta + '', name),
    value,
  )
